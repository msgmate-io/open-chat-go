...
func GetServerFlags() []cli.Flag {
	flags := []cli.Flag{
		...
		&cli.IntFlag{
			Sources: cli.EnvVars("BROWSER_TOKEN_MAX_ACTIVE_PER_USER"),
			Name:    "browser-token-max-active-per-user",
			Usage:   "maximum number of active browser tokens per user; oldest browser tokens are evicted when the limit is exceeded",
			Value:   10,
		},
	}
...
func runServer(ctx context.Context, c *cli.Command) error {
	...
	gitTriggerSpecs := append([]string{}, openChatBootstrap.GitTriggerSpecs...)
	if err := applyGitBootstrapSources(DB, adminUser.Username, gitDefaultOwners, gitTokenSpecs, gitRepositorySpecs, gitWorkspaceSpecs, gitWorkspaceGrantSpecs, gitTriggerSpecs); err != nil {
		return err
	}
